...
export default function CreateResume() {
  ...
  const [foreinLanguages, setForeinLanguages] = useState("")
  ...
  return (
    <main>
      ...
      <div className='container p7'>
        ...
        <AutoCompliteTegs placholder='' type='text' label='Ключевые навыки' size='fieldset-md' items={allSkills} onSelect={onSkillsChange}/>
        
        <h3>Образование</h3>
        <AddEducation onChange={(eds) => setEducation(eds)}/>

        <h3>Владение языками</h3>
        <AddLang onChange={(lns) => setForeinLanguages(lns)}/>

        <h3>Другая важная информация</h3>
        <SelectEmploymentTypes label="Занятость" size="fieldset-md" employmentTypes={allEmploymentTypes} onChange={(tps) => (setSelectedEmptTypes(tps))}/>
